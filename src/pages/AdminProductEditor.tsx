...
export default function AdminProductEditor(){
 ...
 useEffect(()=>{(async()=>{if(!supabase){setError("Supabase тохируулаагүй байна.");setLoading(false);return}
   ...
   if(!creating){const {data,error}=await supabase.from("products").select("name,slug,internal_reference,short_description,description,material,dimensions,category_id,status,featured,custom_order_available,custom_order_note,display_order").eq("id",id!).single();
     ...
     const {data:v,error:ve}=await supabase.from("product_variants").select("id,name,color,sku,price,active,made_to_order,lead_time_days,display_order,inventory(quantity_on_hand,quantity_reserved,low_stock_threshold,track_inventory)").eq("product_id",id!).order("display_order"); if(ve)setError(ve.message); else setVariants(((v??[]) as any[]).map(row=>{const inv=Array.isArray(row.inventory)?row.inventory[0]:row.inventory;return {...row,quantity_on_hand:inv?.quantity_on_hand??0,quantity_reserved:inv?.quantity_reserved??0,low_stock_threshold:inv?.low_stock_threshold??2,track_inventory:inv?.track_inventory??true}}));
 ...
 async function saveVariants(productId:string){
   ...
   for(const v of variants){
     ...
     if(v.id){const {error}=await supabase.from("product_variants").update(payload).eq("id",v.id);if(error)return error;const {data:old}=await supabase.from("inventory").select("quantity_on_hand").eq("variant_id",v.id).single();const oldQty=old?.quantity_on_hand??0;const newQty=Number(v.quantity_on_hand)||0;const {error:ie}=await supabase.from("inventory").update({quantity_on_hand:newQty,low_stock_threshold:Number(v.low_stock_threshold)||0,track_inventory:v.track_inventory,updated_at:new Date().toISOString()}).eq("variant_id",v.id);if(ie)return ie;if(newQty!==oldQty){const {error:me}=await supabase.from("stock_movements").insert({variant_id:v.id,quantity_change:newQty-oldQty,movement_type:"adjustment",reason:"Админ тохируулга"});if(me)return me}}else{const {data:created,error}=await supabase.from("product_variants").insert(payload).select("id").single();if(error)return error;if(created){const {error:ie}=await supabase.from("inventory").update({quantity_on_hand:Number(v.quantity_on_hand)||0,low_stock_threshold:Number(v.low_stock_threshold)||0,track_inventory:v.track_inventory,updated_at:new Date().toISOString()}).eq("variant_id",created.id);if(ie)return ie;if(Number(v.quantity_on_hand)){const {error:me}=await supabase.from("stock_movements").insert({variant_id:created.id,quantity_change:Number(v.quantity_on_hand),movement_type:"adjustment",reason:"Анхны нөөц"});if(me)return me}}}
   } return null;
 }
 ...
 <form id="product-form" className="admin-editor" onSubmit={save}>
  ...
  <section className="admin-panel admin-variants-panel"><div className="admin-section-head"><div><h2>Хувилбарууд</h2><p>Өнгө, үнэ болон SKU кодыг энд удирдана.</p></div><button type="button" className="admin-secondary" onClick={addVariant}>+ Хувилбар нэмэх</button></div>
   <div className="admin-variants">{variants.map((v,i)=>v._deleted?null:<div className="admin-variant" key={v.id??i}>
    ...
    <div className="admin-stock-fields"><label>Нөөц<input type="number" min="0" value={v.quantity_on_hand} onChange={ev=>updateVariant(i,{quantity_on_hand:Number(ev.target.value)})}/></label><label>Захиалсан<input type="number" value={v.quantity_reserved} disabled /></label><label>Бага нөөцийн босго<input type="number" min="0" value={v.low_stock_threshold} onChange={ev=>updateVariant(i,{low_stock_threshold:Number(ev.target.value)})}/></label></div><div className="admin-variant-options"><label className="admin-check"><input type="checkbox" checked={v.track_inventory} onChange={ev=>updateVariant(i,{track_inventory:ev.target.checked})}/> Нөөц хянах</label><label className="admin-check"><input type="checkbox" checked={v.active} onChange={e=>updateVariant(i,{active:e.target.checked})}/> Идэвхтэй</label><label className="admin-check"><input type="checkbox" checked={v.made_to_order} onChange={e=>updateVariant(i,{made_to_order:e.target.checked})}/> Захиалгаар хийх</label>{v.made_to_order&&<label>Хийх хугацаа (хоног)<input type="number" min="1" value={v.lead_time_days??""} onChange={e=>updateVariant(i,{lead_time_days:e.target.value?Number(e.target.value):null})}/></label>}</div>
