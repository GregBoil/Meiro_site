...
export default function AdminProductEditor(){
 ...
 async function attachMedia(media:{id:string;storage_path:string;filename:string;alt_text:string|null}){if(!supabase||!id)return;if(images.some(i=>i.media_id===media.id)){setError("Энэ зураг бүтээгдэхүүнд аль хэдийн нэмэгдсэн байна.");return}const {data,error}=await supabase.from("product_images").insert({product_id:id,media_id:media.id,is_primary:images.length===0,display_order:images.length}).select("id,variant_id,media_id,is_primary,display_order").single();if(error||!data){setError(error?.message||"Зураг холбож чадсангүй.");return}setImages(x=>[...x,{...data,media} as ProductImage]);setShowLibrary(false)}
